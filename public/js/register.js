/**
 * Tutorly JavaScript - register.js
 * Handles mobile menu (hidden by default), dark mode, smooth scrolling, and registration form validation.
 */
...
const initFormValidation = () => {
    ...
    const validateField = (field) => {
        ...
        if (!value) {
            setError(field, `${name.replace('-', ' ')} is required`);
            return false;
        }
        ...
        if (name === 'username') {
            ...
            if (!field.checked) {
                setError(field, 'You must agree to the terms');
                return false;
            }
        }
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (isValid) {
            ...
            // });
            // alert('Registration successful! (Mock action)');
            // form.reset();
